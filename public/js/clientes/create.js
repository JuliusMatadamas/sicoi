...
let form = document.querySelector("#form-nuevo_cliente");
let idCliente = generateUniqueID();
...
function cancelarRegistro(){
    ...
    document.querySelector("#modal-confirm_cancel").style.display = 'none';
}
    ...
    grecaptcha.execute('6LftkdAiAAAAACOhv43G34x3CFqmKiBPgkGKyjkB', {
        ...
        xhttp.onreadystatechange = function () {
            if (xhttp.readyState === 4 && xhttp.status === 200) {
                ...
                cancelarRegistro();
                document.querySelector("#modal-confirm_cliente").style.display = 'flex';
                document.querySelector("#btn-venta").addEventListener("click", e => {
                    window.location = entorno.value + '/ventas/nueva?id=' + idCliente;
                });
                document.querySelector("#btn-nuevo").addEventListener("click", e => {
                    idCliente = generateUniqueID();
                });
            } else {
                if (xhttp.status === 400) {
                    let res = JSON.parse(xhttp.responseText);
