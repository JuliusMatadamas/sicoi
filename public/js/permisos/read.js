let cols = document.querySelectorAll(".a-permiso");
let d = new Date();
let month = d.getMonth() + 1;
if ( month < 10 ) month = '0' + month;
let currentDate = `${d.getFullYear()}-${month}-${d.getDate()}`;

let form = document.querySelector("#form-edit_permiso");
...
setTextAlignToLinkPermiso();

window.addEventListener('resize', () => {
    setTextAlignToLinkPermiso();
});

function setTextAlignToLinkPermiso() {
    if (window.innerWidth < 992) {
        cols[1].parentElement.classList.remove('text-end');

        cols[0].parentElement.classList.add('text-center');
        cols[1].parentElement.classList.add('text-center');
    } else {
        cols[0].parentElement.classList.remove('text-center');
        cols[1].parentElement.classList.remove('text-center');

        cols[1].parentElement.classList.add('text-end');
    }
}
